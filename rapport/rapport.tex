...
% TODO (lire doc Thweatt1999.pdf) mu=2 -> mu = 2.5 (courbe en 8): changement de dynamique : bifurcation
...
\end{lstlisting}
% TODO explications (meshgrid + contour)
\newpage
...
\begin{flalign*}
	...
	&= -\cos{x_1}\left( 1 - \frac{2}{\sqrt{\mu^2 +1 -2\mu\cos{x_1}}} \right) -2\mu\frac{\sin{x_1}^2}{(\mu^2 +1 -2\mu\cos{x_1})^{3/2}}&
...
\underline{On fixe le point d'équilibre à $x^* = (0,0)$}:\\
\begin{flalign*}
	&\begin{cases}
		tr(A) = -\alpha\\
		det(A) = -\frac{\partial f_2}{\partial x_2} = 1 - \frac{2}{\sqrt{\mu^2 +1 -2\mu}}\\
		\delta(A) = tr(A)^2 + 4*det(A) = \alpha^2 - 4\left( 1 - \frac{2}{\sqrt{\mu^2 +1 -2\mu}} \right)
	\end{cases}&
\end{flalign*}
